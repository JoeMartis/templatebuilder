...
async function buildDocx(cfg) {
  ...
  for (let l = 1; l <= cfg.lectures; l++) {
    lecturesSection.push(
      ...
      h3(`Lecture ${l} Videos`),
      sp(60),
      ...(cfg.inclInstr ? [noteBox(`This lecture has ${(cfg.segmentsPerLecture||[])[l-1]||cfg.segments||3} segment${((cfg.segmentsPerLecture||[])[l-1]||cfg.segments||3)!==1?'s':''}. Fill in the script/outline for each, then complete the Knowledge Check${((cfg.kcsPerLecture||[])[l-1]??cfg.kcs??1)>1?'s':''} that follow.`), sp(80)] : []),
    );

    // Segments
    const numSegs_l = (cfg.segmentsPerLecture||[])[l-1] || cfg.segments || 3;
    const kcForLecture_l = (cfg.kcsPerLecture||[])[l-1] != null ? (cfg.kcsPerLecture||[])[l-1] : (cfg.kcs != null ? cfg.kcs : 1);
    for (let s = 1; s <= numSegs_l; s++) {
      const kcTotal = kcForLecture_l;
      lecturesSection.push(
        sp(80),
        new Paragraph({spacing:{before:0,after:80},
  ...
  // ════════════════════════════════════════════════════════════
  const concludeNum = sectionNum + (cfg.hasRecit?1:0) + 2;
  const conclusion = [
