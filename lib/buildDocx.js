...
async function buildDocx(cfg) {
  ...
  for (let l = 1; l <= cfg.lectures; l++) {
    lecturesSection.push(
      ...
      h3(`Lecture ${l} Videos`),
      sp(60),
      ...(cfg.inclInstr ? [noteBox(`This lecture has ${(cfg.segmentsPerLecture||[])[l-1]||3} segment${((cfg.segmentsPerLecture||[])[l-1]||3)!==1?'s':''}. Fill in the script/outline for each, then complete the Knowledge Check(s) that follow each segment.`), sp(80)] : []),
    );
