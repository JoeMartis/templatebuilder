...
async function buildDocx(cfg) {
  ...
  for (let l = 1; l <= cfg.lectures; l++) {
    lecturesSection.push(
      ...
      h3(`Lecture ${l} Videos`),
      sp(60),
      ...(cfg.inclInstr ? [noteBox(`This lecture has ${cfg.segments} segment${cfg.segments!==1?'s':''}. Fill in the script/outline for each, then complete the Knowledge Check${cfg.kcs>1?'s':''} that follow.`), sp(80)] : []),
    );
    ...
    for (let s = 1; s <= cfg.segments; s++) {
      ...
      lecturesSection.push(
        sp(80),
        new Paragraph({spacing:{before:0,after:80},
          children:[txt(`Video Segment ${s}`,{bold:true,size:22,color:MED})]}),
    ...
    for (let r = 1; r <= cfg.recitations; r++) {
      ...
      for (let s = 1; s <= cfg.recitSegs; s++) {
        recitSection.push(
          sp(80),
          new Paragraph({spacing:{before:0,after:80}, children:[
            txt(`Video Segment ${s}`,{bold:true,size:22,color:MED})
          ]}),
