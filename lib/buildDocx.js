...
async function buildDocx(cfg) {
  ...
  const cover = [
    ...
    sp(80),
    fieldBox("Estimated Completion Time", cfg.time || "[Estimated time]"),
    sp(80),
    fieldBox("Target Audience / Prerequisites", cfg.prereqs || "[Prerequisites]"),
    sp(400),
    ...(cfg.inclInstr ? [
      ...
      body(`3. This template includes ${cfg.lectures} lecture${cfg.lectures!==1?'s':''}, ${cfg.hasRecit?cfg.recitations+' recitation'+(cfg.recitations!==1?'s':''):'no recitations'}, and ${cfg.hasAssign?cfg.assignments+' assignment'+(cfg.assignments!==1?'s':''):'no assignments'}.`),
  ...
  for (let l = 1; l <= cfg.lectures; l++) {
    lecturesSection.push(
      ...
      h3(`Lecture ${l} Videos`),
      sp(60),
      ...(cfg.inclInstr ? [noteBox(`This lecture has ${(cfg.segmentsPerLecture||[])[l-1]||cfg.segments||3} segment${((cfg.segmentsPerLecture||[])[l-1]||cfg.segments||3)!==1?'s':''}. Fill in the script/outline for each, then complete the Knowledge Check${cfg.kcs>1?'s':''} that follow.`), sp(80)] : []),
    );

    // Segments
    const numSegs_l = (cfg.segmentsPerLecture||[])[l-1] || cfg.segments || 3;
    for (let s = 1; s <= numSegs_l; s++) {
      const kcTotal = cfg.kcs;
      lecturesSection.push(
        sp(80),
        new Paragraph({spacing:{before:0,after:80},
        ...
        sp(80),
        fieldBox("Target Duration", "e.g., 5–7 minutes", GREEN_BG),
        sp(80),
        bigFieldBox("Script / Content Outline", [
          "Write your script or a detailed outline here.",
          "",
          "Suggested structure:",
          "  • Hook / opening question",
          "  • Core concept explanation",
          "  • Example or analogy",
          "  • Visual / slide notes (describe what should appear on screen)",
          "  • Transition to next segment",
        ], GREEN_BG, "Slides, diagrams, or code can be referenced here and attached separately."),
      );
    ...
    for (let r = 1; r <= cfg.recitations; r++) {
      ...
      for (let s = 1; s <= cfg.recitSegs; s++) {
        recitSection.push(
          ...
          sp(80),
          fieldBox("Estimated Duration", "e.g., 15 minutes", PURPLE_BG),
          sp(80),
          bigFieldBox("Script / Content Outline", [
            "Detailed outline or script.",
            "",
  ...
  // ════════════════════════════════════════════════════════════
  const concludeNum = sectionNum + (cfg.hasRecit?1:0) + (cfg.hasAssign?1:0) + 1;
  const conclusion = [
    ...
    h2(`What's Next (Optional)`),
    sp(60),
    bigFieldBox("Preview of Next Module", [
      "1–2 sentences previewing the next module or topic.",
      "Example: In the next module, you will build on these foundations to explore more advanced techniques.",
    ], LIGHT),
    sp(200),
  ];
